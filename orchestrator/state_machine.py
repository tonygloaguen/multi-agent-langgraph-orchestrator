...
def _scan_secrets(repo_path: str) -> tuple[bool, str]:
    if subprocess.run(
        "which gitleaks", shell=True, capture_output=True
    ).returncode != 0:
        return True, "gitleaks absent — scan ignoré"
    rc, out, _ = _cmd(
        f"gitleaks detect --source {repo_path} --no-git --redact -q",
        cwd=repo_path
    )
...
def node_init(state: OrchestratorState) -> dict:
    ...
    _log_event(run_id, "pipeline_start", {
        "repo_path": repo_path,
        "goal":      state.get("goal", ""),
    }, Path(log_dir))
...
def node_preflight(state: OrchestratorState) -> dict:
    ...
    if errors:
        ...
        _log_event(state["run_id"], "preflight_failed",
                   {"errors": errors}, Path(state["log_dir"]))
...
def node_snapshot(state: OrchestratorState) -> dict:
    ...
    _log_event(state["run_id"], "snapshot_created",
               {"snapshot_hash": snap_hash}, Path(state["log_dir"]))
...
def node_plan(state: OrchestratorState) -> dict:
    ...
    try:
        ...
        _log_event(state["run_id"], "plan_generated", {
        ...
        _log_event(state["run_id"], "planning_failed",
                   {"error": str(e)}, Path(state["log_dir"]))
...
def node_prepare_task(state: OrchestratorState) -> dict:
    ...
    _log_event(state["run_id"], "task_start", {
        "task_id":       task_id,
...
def node_implement(state: OrchestratorState) -> dict:
    ...
    if not result["success"]:
        ...
        _log_event(state["run_id"], "task_impl_failed", {
...
def node_analyze(state: OrchestratorState) -> dict:
    ...
    try:
        ...
        if analysis.get("escalate"):
            ...
            _log_event(state["run_id"], "task_escalated", {
                "task_id":    state["handoff"]["task_id"],
...
def node_repair(state: OrchestratorState) -> dict:
    ...
    console.print(f"[yellow]Repair {attempts}/{cfg.orchestrator_max_repair_loops}...[/yellow]")
    ...
    if result.get("regressed"):
        ...
        _log_event(state["run_id"], "repair_regression", {
            "task_id": state["handoff"]["task_id"],
            "attempt": attempts,
        }, Path(state["log_dir"]))
...
def node_commit(state: OrchestratorState) -> dict:
    ...
    _log_event(state["run_id"], "task_completed", {
        "task_id":          task_id,
...
def node_done(state: OrchestratorState) -> dict:
    ...
    _log_event(state["run_id"], "pipeline_completed", {
        "completed": completed,
        "total":     total,
    }, Path(state["log_dir"]))
...
def route_after_validate(
    ...
    if state.get("repair_attempts", 0) >= cfg.orchestrator_max_repair_loops:
        console.print(f"[red]Max repairs atteint[/red]")
...
def build_graph() -> Any:
    ...
    g.add_conditional_edges("review", route_after_review,
                             {"commit": "commit", "next_or_done": "skip_task"})
    g.add_conditional_edges("commit", route_after_commit,
                             {"prepare_task": "prepare_task", "done": "done"})
    g.add_conditional_edges("skip_task", route_next_or_done,
                             {"prepare_task": "prepare_task", "done": "done"})
